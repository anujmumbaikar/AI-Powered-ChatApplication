...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const { sendMessage: streamSendMessage } = useChannelActionContext();
  ...
  const textareaRef = useRef<HTMLTextAreaElement>(null) as React.RefObject<HTMLTextAreaElement>;
    ...
    const handleStopGenerating = () => {
      if (channel) {
        const aiMessage = [...(messages ?? [])]
          .reverse()
          .find((m) => m.user?.id && m.user.id.startsWith("ai-bot"));
        if (aiMessage) {
          channel.sendEvent({
    ...
    // Wrapper to match ChatInput's expected sendMessage signature
